#!/usr/bin/env python3
"""
Script to generate a refactored version of the segmenter-old notebook.
...
def add_header_section(nb):
    ...
    nb.cells.append(new_markdown_cell(
        ...
        "Character-level BiLSTM for Quechua morphological segmentation. "
        "Predicts boundary positions at the character level, marking where "
...
gold_df['word_len'] = gold_df['Word'].apply(len)'''
    ...
    boundary_labels = '''def get_boundary_labels(word, split):
...
def add_loss_section(nb):
    ...
    loss_code = '''def masked_bce_loss(logits, targets, mask):
...
def add_inference_section(nb):
    ...
    inference_code = '''def predict_boundaries(words: List[str], model, stoi, threshold=0.5) -> List[List[int]]:
...
def add_evaluation_section(nb):
    ...
    eval_helpers = '''def boundary_positions_from_labels(labels):
...
def main():
    ...
    # Save notebook
    output_path = "segmenter-old_refactored.ipynb"
